    ...
    def _update_bucket_weights_from_remote(
        ...
        for key, transfer_bundle in self.engines.items():
            ...
            # if any([("expert" in n) for n in ds]):
                # logger.info(f"[for target rank {key}] expert transfering: {ds}. transfer_bundle.model_replica. ep size {transfer_bundle.model_replica.ep_size}, num_local_experts {transfer_bundle.model_replica.config.num_experts}")
...
class MockSglangDistributedContext:
    ...
    def __enter__(self):
        ...
        self._patches = [
            ...
            patch("sglang.srt.models.qwen3_moe.get_attention_tp_rank", return_value=self.attn_tp_rank),
            patch("sglang.srt.models.qwen3_moe.get_moe_expert_parallel_rank", return_value=self.ep_rank),
